...
humans = [
	Human(
		title='Human #1',
		work_day=date(2022, 10, 11),
		work_type=work_graphs['1/3']
	),
	Human(
		title='Human #2',
		work_day=date(2022, 10, 10),
		# work_day=date(2022, 10, 24),
		work_type=work_graphs['5/2']
	),
]

# f = Filter(humans)
# hms = list(map(str, f.get_working(datetime.now())))
# print(hms)
...
class TestFilter(unittest.TestCase):
	...
	def test_get_wk_human_1(self):
		...
		h_val = (date(2022, 10, 11), date(2022, 10, 14))
		self.assertEqual(h_wk, h_val)

	def test_get_wk_human_2(self):
		h_wk = self.filter._get_wk(self.human2)
		h_val = (date(2022, 10, 10), date(2022, 10, 16))
		self.assertEqual(h_wk, h_val)

	def test_wk_bias_human_1(self):
		wk = (date(2022, 10, 11), date(2022, 10, 14))
		self.filter._get_working_wk(wk, self.human1.work_day)

	def test_wk_bias_human_2(self):
		wk = (date(2022, 10, 10), date(2022, 10, 16))
		self.filter._get_working_wk(wk, self.human2.work_day)
